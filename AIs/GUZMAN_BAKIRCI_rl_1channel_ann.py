...
class MultiRegressor2FC(nn.Module):
    ...
    def load(self):
        if self.nb_channels == 1:
            self.load_state_dict(torch.load('AIs/weights_ANN2FC_1channel.pt'))
        else:
            self.load_state_dict(torch.load('AIs/weights_ANN2FC_2channel.pt'))

    def save(self):
        if self.nb_channels == 1:
            torch.save(self.state_dict(), 'AIs/weights_ANN2FC_1channel.pt')
        else:
            torch.save(self.state_dict(), 'AIs/weights_ANN2FC_2channel.pt')
...
def preprocessing(mazeMap, mazeWidth, mazeHeight, playerLocation, opponentLocation, piecesOfCheese, timeAllowed):
    ...
    action = -1
    score = 0
    model = MultiRegressor2FC(input_tm1[0], number_of_channels=1)
